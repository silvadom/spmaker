...
int main(int argc, char** argv)
{
    ...
    flat_map<std::string,TickerData> lastTickersMap;
    ...
    workers.push_back(std::thread([&]()
    {
        ...
        while(running)
        {
            ...
            for(size_t j=0; j<n; ++j)
            {
                ...
                // cache last ticker by instrument and asset class
                std::string key = ticker.instrum;
                key.append(ticker.assetClass);
                lastTickersMap.insert_or_assign(key, ticker);
                ...
                /** ##############################################################
                 * @brief Implement very simple spot-futures arbitrage strategy
                 * ###############################################################
                 */
                std::string spotkey = ticker.instrum;
                std::string futkey = ticker.instrum;
                spotkey.append("spot");
                futkey.append("future");
                ...
                */
                if(lastTickersMap.count(spotkey) && lastTickersMap.count(futkey))
                {
                    const TickerData& spotTicker = lastTickersMap.at(spotkey);
                    const TickerData& futTicker = lastTickersMap.at(futkey);
                    spread = (futTicker.bid - futTicker.indexPrice) / futTicker.bid * 100.0;
                    hasTickers = true;
                }

                /**
                 * OPEN A SHORT POSITION if funding rate is 0.05% and above
                 */           
                if(!hitThreadLimit && hasTickers && hasBalance && 
                    abs(spread) >= 0.0005 && !hitRequestLimit)
                {
                    ++usedCounter;
                    ...
                    boost::asio::dispatch(pool, [&, ticker]()
                    {
                        ...
                        postOrder["side"] = (spread > 0.0005) ? "SELL" : "BUY"; 
                ...
                if(hasPosition && !hitRequestLimit)
                {
                    ...
                    for(const OrderData& order: orders)
                    {
                        ...
                        /**
                         * CLOSE A SHORT POSITION when funding rates decline
                        */
                        if(spread <= 0.0001 && hasTickers && !execManager.IsClosingRequested(order))
                        {
                            //++usedCounter;
                            ...
                            boost::asio::dispatch(pool, [&, ticker, order]()
                            {
                                ...
                                reduceOrder["side"] = (order.side == "BUY") ? "SELL": "BUY";
